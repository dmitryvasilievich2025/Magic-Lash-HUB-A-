...
import { Save, Play, Wand2, ChevronDown, BookOpen, MessageSquare, Zap, Film, Loader2, Sparkles, RefreshCw, CheckCircle2, Mic, MicOff, Settings2, Video, Edit3, Layout, Plus, Link, Image as ImageIcon } from 'lucide-react';
...
const CourseEditor: React.FC<Props> = ({ course, onUpdate, lang }) => {
  ...
  const placeholders = {
    ...
    mediaUrl: lang === 'uk'
      ? "Вставте URL відео або зображення..."
      : "Paste video or image URL..."
  };
  ...
  const handleOptimizePrompt = async () => {
    ...
    try {
      const ai = getAI();
      const systemInstruction = `Ти — провідний технолог та експерт з ${isExtension ? 'нарощування вій Magic Lash' : 'ламінування вій InLei'}. 
      Твоя задача: сформувати професійний ТЕХНІЧНИЙ ПРОМПТ для візуалізації "${targetTitle}".
      
      Результат має бути лаконічним технічним описом українською мовою.`;
      
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Сформуй професійний технічний опис для візуалізації: ${targetTitle}`,
        config: { systemInstruction }
      });
      
      if (response.text) {
        if (activeStepId === 'lesson') updateActiveLesson('aiPrompt', response.text.trim());
        else updateActiveStep('aiPrompt', response.text.trim());
      }
    } catch (error) {
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      const visualPrompt = `
        Educational masterclass cinematography for beauty industry.
        Subject: ${targetPrompt}.
        Scientific Context: ${targetRag || 'macro focus on technical precision'}.
        Vibe: 4K resolution, cinematic studio lighting, extreme macro lens.
        Focus: ${isExtension ? 'eyelash attachment geometry and adhesive placement' : 'lash lamination chemical process and hair structure transformation'}.
        Style: Clinical, high-end, clean background, 24fps.
      `;
      ...
      alert(lang === 'uk' ? "Помилка генерації відео через Veo. Перевірте API ключ або промпт." : "Error generating video with Veo. Check API Key or Prompt.");
    } finally {
        ...
        <div className="text-left min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-[10px] font-black uppercase tracking-widest text-gray-500 truncate">
              {activeStepId === 'lesson' ? (lang === 'uk' ? 'Редагування Заняття' : 'Lesson Overview') : (lang === 'uk' ? 'Редагування Етапу' : 'Step Editor')}
            </h2>
          </div>
          <p className="text-xl font-black text-gray-100 truncate">{activeStepId === 'lesson' ? activeLesson.title : activeStep?.title}</p>
        </div>
        <div className="flex gap-3 shrink-0">
           <button className={`px-5 py-2.5 ${brandBg} text-white rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-lg transition-all`}>
               ...
               {activeLessonId === lesson.id && (
                 <div className="space-y-2 pl-4">
                   ...
                   >
                     ...
                     <span className="text-[10px] font-bold text-white tracking-tight">{lang === 'uk' ? 'Вступне Відео' : 'Intro Video'}</span>
                   </div>
           ...
           <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
              <div className="space-y-2">
                 <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">
                    {activeStepId === 'lesson' ? (lang === 'uk' ? 'Заголовок блоку' : 'Block Heading') : (lang === 'uk' ? 'Назва технічного кроку' : 'Technical Step Name')}
                 </label>
                 <input 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-[#12141C] p-8 rounded-[3rem] border border-[#1F232B] shadow-sm space-y-6 text-left relative overflow-hidden group">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={toggleListening} className={`p-2 rounded-lg transition-all ${isListening ? 'bg-red-500 text-white animate-pulse' : 'bg-white/5 text-gray-400 border border-white/5'}`}>
                          {isListening ? <MicOff size={14} /> : <Mic size={14} />}
                        </button>
                        <button onClick={handleOptimizePrompt} disabled={isOptimizingPrompt} className="flex items-center gap-1.5 px-3 py-1 bg-white/5 hover:bg-white/10 rounded-lg text-[8px] font-black uppercase text-gray-400 border border-white/5">
                          {isOptimizingPrompt ? <Loader2 size={10} className="animate-spin" /> : <Sparkles size={10} className={brandAccent} />}
                          ARI AUTO
                        </button>
                      </div>
                    </div>
                ...
                {/* Video Generation Module (Veo) */}
                <div className="bg-[#12141C] p-8 rounded-[3rem] border border-[#1F232B] shadow-sm flex flex-col space-y-6 relative overflow-hidden">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 flex items-center gap-2">
                      <Film size={14} className={brandAccent} /> {activeStepId === 'lesson' ? 'Intro Video (Veo 3.1)' : 'Step Video (Veo 3.1)'}
                    </label>
                  </div>
                  ...
                  <div className="flex-1 min-h-[160px] bg-[#0A0C10] rounded-3xl border border-[#1F232B] overflow-hidden flex flex-col items-center justify-center relative group shadow-inner">
                    {isGeneratingVideo ? (
                      <div className="flex flex-col items-center gap-6 p-6 text-center animate-pulse">
                        ...
                        <p className="text-[10px] font-black text-white uppercase tracking-widest leading-relaxed px-4">
                      ...
                      <div className="w-full h-full relative group/vid">
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/vid:opacity-100 transition-all flex items-center justify-center">
                           <button onClick={handleGenerateVideo} className="p-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white shadow-2xl hover:scale-110 transition-transform">
                             <RefreshCw size={24} />
                           </button>
                        </div>
                      </div>
                      ...
                      <div className="text-center p-6 space-y-4 opacity-30 group-hover:opacity-60 transition-opacity">
                        <div className="w-16 h-16 bg-[#12141C] border border-[#1F232B] rounded-2xl flex items-center justify-center mx-auto">
                          <Video size={32} />
                        </div>
                  ...
                  <button 
                    ...
                    className={`w-full py-5 ${brandBg} text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.25em] transition-all flex items-center justify-center gap-3 disabled:opacity-50 hover:brightness-110 active:scale-95 shadow-xl`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {/* Media Link / URL Block */}
                <div className="bg-[#12141C] p-10 rounded-[3.5rem] border border-[#1F232B] shadow-sm space-y-6 text-left relative overflow-hidden group">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 flex items-center gap-2">
                      <Link size={14} className={brandAccent} /> {lang === 'uk' ? 'Медіа-ресурс (URL)' : 'Media URL'}
                    </label>
                    <div className="space-y-4">
                      <div className="relative group/input">
                         <ImageIcon size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within/input:text-purple-500 transition-colors" />
                         <input 
                            className={`w-full bg-[#0A0C10] border border-[#1F232B] rounded-2xl py-5 pl-14 pr-8 text-xs font-bold text-gray-100 outline-none focus:ring-1 ${isExtension ? 'ring-purple-500/30' : 'ring-yellow-500/30'} transition-all placeholder-gray-800`}
                            value={currentMedia || ''}
                      ...
                      <p className="text-[9px] font-black uppercase text-gray-600 tracking-widest leading-relaxed">
                        {lang === 'uk' ? 'Вкажіть пряме посилання на відео (.mp4) або зображення. Цей ресурс буде використано для відображення студенту.' : 'Specify a direct link to a video (.mp4) or an image. This resource will be used for student viewing.'}
                      </p>
                    </div>
                </div>
              </div>
