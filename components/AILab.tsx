...
import { Film, Image as ImageIcon, Sparkles, Send, Loader2, Play, Download, Zap, Info, Plus, Columns, Target, Scissors, ChevronRight, BarChart3, Star, ClipboardCheck, CheckCircle2, AlertTriangle, Fingerprint, Award, Activity } from 'lucide-react';
...
const AILab: React.FC<Props> = ({ lang }) => {
  ...
  const t = {
    uk: {
      ...
      ПОРІВНЯЙ фото ДО (вихідний стан, анатомія) та ПІСЛЯ (результат).
      ...
      Надайте порівняльний аналіз: чи відповідає обраний ефект анатомії клієнта на фото "ДО".
      Надай детальні практичні поради рівня "Master-Level" для покращення техніки на основі виявлених помилок.`,
      uploadBefore: 'Фото ДО (Анатомія)',
      uploadAfter: 'Фото ПІСЛЯ (Робота)',
      magic: 'ARI проводить технічний аудит...',
      generate: 'Запустити Майстер-Аналіз',
      auditResult: 'Протокол Технічного Аудиту',
      contextPlaceholder: "Опишіть ефект (напр. 'Лисичка', L-вигин), щоб ARI врахувала це при оцінці...",
      scores: "Технічні Бали",
      strengths: "Сильні сторони",
      improvements: "Критичні зауваження",
      advice: "Вердикт Експерта ARI",
      comparisonBadge: "Deep Comparison Active"
    }
  }[lang] || { uk: {} };
  ...
  const processTool = async () => {
    ...
    try {
      if (activeTool === 'video') {
        const videoBlob = await generateEducationalVideo(prompt, previewImage?.split(',')[1]);
        ...
        if (images.length === 0) throw new Error("Завантажте фото для аналізу");
  ...
  const chartData = useMemo(() => {
    ...
    return [
      { subject: 'Симетрія', score: symmetry, fullMark: 10 },
      { subject: 'Напрямок', score: direction, fullMark: 10 },
      { subject: 'Чистота', score: cleanliness, fullMark: 10 },
    ];
  }, [analysisResult]);
    ...
    <div className="flex-1 flex flex-col bg-[#0A0C10] overflow-hidden animate-in fade-in duration-700">
      <header className="h-20 bg-[#12141C] border-b border-[#1F232B] flex items-center justify-between px-10 shrink-0">
        <div className="text-left">
          ...
          <p className="text-xl font-black text-white uppercase tracking-tight">{t.audit}</p>
        </div>
      ...
      <div className="flex-1 p-10 overflow-y-auto custom-scrollbar">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 space-y-8 text-left">
            <div className={`bg-[#12141C] p-10 rounded-[3.5rem] border border-[#1F232B] space-y-8 shadow-2xl transition-all ${activeTool === 'work-audit' ? 'border-orange-500/20 ring-1 ring-orange-500/5' : ''}`}>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                   ...
                   {activeTool === 'work-audit' && <span className="text-[8px] font-black uppercase bg-orange-500/10 text-orange-400 px-2 py-1 rounded-md tracking-tighter animate-pulse">{t.comparisonBadge}</span>}
                </div>
                <textarea 
                    ...
                    >
                      {previewImage ? (
                        ...
                        <div className="text-center space-y-2">
                           ...
                           <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest">Select Anatomy</span>
                        </div>
                      )}
                      ...
                      >
                        {afterImage ? (
                          ...
                          <div className="text-center space-y-2">
                             ...
                             <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest">Select Work</span>
                          </div>
                        )}
          ...
          <div className="lg:col-span-7">
            <div className="bg-[#12141C] rounded-[4rem] border border-[#1F232B] p-12 min-h-[600px] flex flex-col shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                 <Fingerprint size={180} className={activeTool === 'work-audit' ? 'text-orange-500' : 'text-purple-500'} />
              </div>
              ...
              {analysisResult ? (
                <div className="w-full h-full text-left space-y-10 overflow-y-auto pr-4 custom-scrollbar animate-in slide-in-from-right-4 relative z-10">
                  <div className="flex items-center gap-4 border-b border-[#1F232B] pb-6">
                     <div className="w-12 h-12 rounded-2xl bg-orange-600/10 flex items-center justify-center text-orange-400 border border-orange-500/20"><Award size={24} /></div>
                     <div><h3 className="text-xl font-black text-white uppercase tracking-tight">{t.auditResult}</h3><p className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Master-Level Technical Protocol</p></div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-[#0A0C10] p-10 rounded-[3rem] border border-white/5 flex flex-col items-center justify-center text-center shadow-inner">
                       <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Загальний Скоринг</p>
                    ...
                    <div className="bg-[#0A0C10] p-4 rounded-[3rem] border border-white/5 shadow-inner">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     ...
                     <div className="space-y-4">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-red-500 flex items-center gap-2"><AlertTriangle size={16} /> {t.improvements}</h4>
                        <div className="space-y-3">
                           {analysisResult.improvements.map((s, i) => <p key={i} className="text-sm font-medium text-gray-300 bg-[#0A0C10] border border-red-500/10 p-4 rounded-2xl">{s}</p>)}
                        </div>
                     </div>
                  </div>

                  <div className="bg-gradient-to-br from-orange-600/10 to-orange-900/10 p-10 rounded-[3.5rem] border border-orange-500/20 space-y-4 relative overflow-hidden group">
                     <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:scale-110 transition-transform"><Sparkles size={40} className="text-orange-400" /></div>
                     <h4 className="text-[10px] font-black uppercase tracking-widest text-orange-400 flex items-center gap-2"><Sparkles size={16} /> {t.advice}</h4>
                     <p className="text-gray-200 text-base leading-relaxed font-medium whitespace-pre-wrap">{analysisResult.advice}</p>
                  </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-6 opacity-20 group-hover:opacity-40 transition-opacity">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Завантажте фото ДО/ПІСЛЯ та запустіть аудит</p>
                  </div>
                </div>
              )}
            </div>
