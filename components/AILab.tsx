...
import { Film, Image as ImageIcon, Sparkles, Send, Loader2, Play, Download, Zap, Info, Plus, Columns, Target, Scissors, ChevronRight, BarChart3 } from 'lucide-react';
...
const AILab: React.FC<Props> = ({ lang }) => {
  const [activeTool, setActiveTool] = useState<'video' | 'analysis' | 'audit'>('audit');
  ...
  const t = {
    uk: {
      video: 'Veo Video',
      vision: 'Vision Pro',
      audit: 'Аналіз Робіт (Audit)',
      auditPrompt: `Ти професійний Lash-аудитор. Проведи технічний аналіз роботи.
      ПОРІВНЯЙ фото ДО та ПІСЛЯ.
      ОБОВ'ЯЗКОВО вистави бали від 1 до 10 за наступними критеріями:
      1. СИМЕТРІЯ (відносно форми обличчя та очей).
      2. НАПРЯМОК ВІЙ (плавність переходу та кути).
      3. ЧИСТОТА СКЛЕЙОК (наявність надлишків клею).
      Надай практичні поради MASTER-LEVEL для покращення техніки.`,
      uploadBefore: 'Фото ДО',
      uploadAfter: 'Фото ПІСЛЯ',
    ...
    en: {
      ...
      vision: 'Vision Pro',
      audit: 'Work Audit',
      auditPrompt: `You are a professional Lash Auditor. Conduct a technical analysis of the work.
      COMPARE the BEFORE and AFTER photos.
      MANDATORY: Give scores from 1 to 10 for:
      1. SYMMETRY (relative to face and eye shape).
      2. LASH DIRECTION (smoothness and angles).
      3. BONDING CLEANLINESS (glue excess).
      Provide MASTER-LEVEL practical advice to improve technique.`,
    ...
    <div className="flex-1 flex flex-col bg-[#0A0C10] overflow-hidden animate-in fade-in duration-700">
      <header className="h-20 bg-[#12141C] border-b border-[#1F232B] flex items-center justify-between px-10 shrink-0">
        <div className="text-left">
          ...
          <p className="text-xl font-black text-white uppercase tracking-tight">Технічний Центр Аудиту</p>
        </div>
        <div className="flex bg-[#0A0C10] p-1.5 rounded-[1.5rem] border border-[#1F232B]">
           <button 
             onClick={() => { setActiveTool('video'); setAnalysisText(null); }}
           ...
           <button 
             onClick={() => { setActiveTool('analysis'); setAnalysisText(null); }}
           ...
           <button 
             onClick={() => { setActiveTool('audit'); setAnalysisText(null); }}
             className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase transition-all flex items-center gap-2 ${activeTool === 'audit' ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-gray-500 hover:text-gray-300'}`}
           >
             <Target size={14} /> {t.audit}
           </button>
        </div>
      </header>
      ...
      <div className="flex-1 p-10 overflow-y-auto custom-scrollbar">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 space-y-8 text-left">
            <div className="bg-[#12141C] p-10 rounded-[3.5rem] border border-[#1F232B] space-y-8 shadow-2xl">
              <div className="space-y-4">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 flex items-center gap-2">
                  <Sparkles size={16} className="text-purple-500" /> 
                  {activeTool === 'audit' ? 'Контекст роботи' : 'Запит для ARI'}
                </h3>
                <textarea 
                  className="w-full h-32 bg-[#0A0C10] border border-[#1F232B] rounded-[2rem] p-6 text-sm font-medium text-white focus:ring-1 ring-purple-500/50 resize-none outline-none placeholder-gray-700 transition-all"
                  placeholder={activeTool === 'audit' ? "Наприклад: L-вигин, 0.07, InLei склад 25.9. Клієнтка просила ефект лисички..." : "Опишіть ваш запит..."}
                  value={prompt}
              ...
              <div className="space-y-6">
                <div className={`grid ${activeTool === 'audit' ? 'grid-cols-2' : 'grid-cols-1'} gap-6`}>
                  <div className="space-y-3">
                    <label className="text-[9px] font-black uppercase text-gray-500 tracking-widest ml-4">{t.uploadBefore}</label>
                    <div 
                      onClick={() => fileInputRef.current?.click()}
                      className="aspect-square bg-[#0A0C10] border-2 border-dashed border-[#1F232B] rounded-[2.5rem] flex flex-col items-center justify-center cursor-pointer hover:border-purple-500/30 transition-all overflow-hidden relative group"
                    >
                      {previewImage ? (
              ...
              <button 
                onClick={processTool}
                disabled={isGenerating || (activeTool === 'audit' && (!previewImage || !afterImage))}
                className={`w-full py-6 text-white rounded-[2rem] font-black uppercase text-xs tracking-[0.3em] shadow-xl transition-all flex items-center justify-center gap-4 disabled:opacity-30 ${activeTool === 'audit' ? 'bg-orange-600 shadow-orange-900/20' : 'bg-purple-600 shadow-purple-900/20'}`}
              >
          ...
          <div className="lg:col-span-7">
            <div className="bg-[#12141C] rounded-[4rem] border border-[#1F232B] p-12 min-h-[600px] flex flex-col shadow-2xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                 <BarChart3 size={180} className={activeTool === 'audit' ? 'text-orange-500' : 'text-purple-500'} />
              </div>
              ...
              {result ? (
                ...
                <div className="w-full h-full text-left space-y-8 overflow-y-auto pr-4 custom-scrollbar animate-in slide-in-from-right-4 relative z-10">
                  <div className={`flex items-center gap-4 ${activeTool === 'audit' ? 'text-orange-400' : 'text-purple-400'}`}>
                    <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-white/5">
                       <Zap size={24} />
                    </div>
                    <div>
                      <h4 className="font-black text-lg uppercase tracking-tight">{t.auditResult}</h4>
                      <p className="text-[9px] font-black uppercase text-gray-500 tracking-[0.3em]">ARI Professional Vision</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#0A0C10] p-10 rounded-[3rem] border border-[#1F232B] text-gray-200 text-sm leading-[2] font-medium whitespace-pre-wrap shadow-inner relative overflow-hidden">
                    <div className={`absolute top-0 left-0 w-1.5 h-full ${activeTool === 'audit' ? 'bg-orange-600' : 'bg-purple-600'}`} />
                    {analysisText}
                  </div>
